...
{
    public class ODObject : IRemoteEntity {
        ...
        public Dictionary<string, string> Data_old {
            get {
        ...
        public static Guid GetGuidFromEntityUrl(string url, string entityName)
        {
            //Match match = Regex.Match(url, @"EntityDataService.svc/" + entityName + @"Collection\(guid'([A-Za-z0-9\-]+)'\)$", RegexOptions.IgnoreCase);
            Match match = Regex.Match(url, @"(?<=EntityDataService.svc/" + entityName + @"Collection\(guid')(.*)(?='\))", RegexOptions.IgnoreCase);
            if (match.Success) {
